...
This is the single place that knows how the FC 26 player data gets loaded
and how queries.sql gets parsed. Both the notebook and the Streamlit
dashboard call `run_query()` against the exact same SQL. The two exceptions
are the KMeans player-archetype clustering and the value-prediction
regression, which live in pandas/scikit-learn in the notebook (and are
re-run, cached, in app.py) -- SQL isn't the right tool for either.
"""
...
CSV_FILE = DATA_DIR / "players_fc26.csv"
